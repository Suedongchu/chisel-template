...
class SatLeftShift(val m: Int, val n: Int) extends Module {
    ...
    io.shiftout := Mux(io.shiftby > m.asUInt(), 0.asUInt(), io.shiftin >> io.shiftby)
}
...
class FPAddStage1(val n: Int) extends Module {
    ...
    val io = IO(new Bundle {
        ...
        val manta = Output(UInt((expWidth+1).W))
        val mantb = Output(UInt((expWidth+1).W))
    ...
    val ext_exp_a = Cat(0.asUInt(1.W), a_wrap.exponent)
    val ext_exp_b = Cat(0.asUInt(1.W), b_wrap.exponent)
    ...
    // b is larger
    when (exp_diff(expWidth) === 1.asUInt()) {
        // absolute value
        reg_mant_shift := -exp_diff(expWidth - 1, 0)
        //mant_shift := (~exp_diff) + 1.asUInt()
        reg_b_larger := true.B
...
class FPAddStage2(val n: Int) extends Module {
    ...
    val io = IO(new Bundle {
        ...
        val manta_out = Output(UInt((expWidth+1).W))
        val mantb_out = Output(UInt((expWidth+1).W))
    ...
    val shifted_mant = Mux(io.mant_shift > (mantWidth + 1).asUInt(),
                           0.asUInt(), smaller_mant >> io.mant_shift)
...
class FPAddStage3(val n: Int) extends Module {
    ...
    val io = IO(new Bundle {
        ...
        val mant_out = Output(UInt((expWidth+1).W))
        val sign_out = Output(Bool())
        val exp_out = Output(UInt(expWidth.W))
    })
    ...
    val manta_ext = Cat(0.asUInt(1.W), io.manta)
    val mantb_ext = Cat(0.asUInt(1.W), io.mantb)
    ...
    when (mant_sum(mantWidth + 1) === 1.asUInt()) {
        when (io.sub) {
...
class FPAddStage4(val n: Int) extends Module {
    ...
    when (io.mant_in === 0.asUInt()) {
        io.mant_out := 0.asUInt()
        io.exp_out := 0.asUInt()
    } .otherwise {
